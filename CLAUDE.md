...
## 배경 타일맵 (실제 항구/바다)
- `Main.gd::_build_tilemap()` — **TileMapLayer**로 NA 타일셋(`assets/sprites/tileset.png`, 16px, 28×40) 조합.
  단일 반복 타일(좌우선·평평경계) 폐기. 셀 32px(16×scale2), z=-3, nearest 필터.
- 아틀라스 좌표(열,행) `_tiles` 딕셔너리: sea=텍스처 바다 변주[(23,7)(22,7)(23,6)(22,9)(23,9)],
  sand=부드러운 모래 오토타일 내부[(21,13)(20,13)(22,13)], grass[(22,11)(23,11)(24,11)],
  stone[(26,11)(27,11)], wood 부두[(25,8)(24,8)], dirt 길[(21,16)].
  ⚠ (18,11)/(19,11)은 **테두리 있는 포장타일** → 모래로 쓰면 네모 박스로 보임(쓰지 말 것).
- `_terrain_at()`: 항구별 타원거리 + `_noise()`(sin 해시) → **유기적 해안선**(직선 X). 중앙은 부두/길.
  예수님 항구만 돌(석조 광장), 나머지 잔디 섬.
- 타일 좌표 식별은 `/tmp/decode.py`(순수 파이썬 PNG 디코더)로 라벨 그리드 크롭 떠서 Read로 눈으로 확인.

## 현재 상태 (2026-06-17)
MVP 수직 슬라이스 완성: 4기항지(창조·타락·예수님[반전]·회복) + 영접 기도 + 배 갈아타기 엔딩.
배경을 **실제 항구/바다 타일맵 조합**으로 교체(위 섹션). E2E로 끝까지 동작 확인(`docs/screenshots/`).
APK 로컬 빌드 성공·서명 검증 완료. CI 워크플로 추가. **다음**: 13기항지 확장, 기항지별 연출 강화,
저해상도 뷰포트로 이펙트 통일, NA FX 오버레이, 다국어(i18n) 연동.
